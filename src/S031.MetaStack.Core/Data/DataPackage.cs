...
#endif
{
	...
	public partial class DataPackage : IDataReader
	{
		...
		private enum ObjectTypeFlag : byte
		{
			Standart = 0,
			External = 1
		}
...
//#endif
				...
				for (; dr.Read();)
				{
					foreach (KeyValuePair<string, ColumnInfo> kvp in colInfo)
					{
						...
						if (value == null || DBNull.Value.Equals(value))
							...
							_dti[MdbTypeMap.GetType(kvp.Value.DataType)].WriteDelegate(bw, value);
					}
				}
				return ms.ToArray();
			}
		...
		static void WriteObject(BinaryWriter bw, object value)
		{
			...
			{
                bw.Write((byte)ObjectTypeFlag.External);
#if SERIALIZEBINARY
				WriteByteArray(bw, MessagePackSerializer.Typeless.Serialize(value));
#else
                //!!! nOptimize this by remove to colInfo
                bw.Write(type.FullName);
                bw.Write(JSONExtensions.SerializeObject(value));
#endif
			}
		}

		static object ReadObject(BinaryReader br)
		{
			ObjectTypeFlag flag = (ObjectTypeFlag)br.ReadByte();
			if (flag == ObjectTypeFlag.Standart)
			{
				MdbType t = (MdbType)br.ReadByte();
				return _dti[t.Type()].ReadDelegate(br);
			}
			else
			{
#if SERIALIZEBINARY
