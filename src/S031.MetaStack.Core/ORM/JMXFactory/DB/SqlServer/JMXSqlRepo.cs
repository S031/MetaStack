...
namespace S031.MetaStack.Core.ORM
{
	public class JMXSqlRepo : JMXRepo
	{
		...
		public JMXSqlRepo(MdbContext mdbContext) : base(mdbContext)
		{
		...
		{
			await TestSysCatAsync().ConfigureAwait(false);
			...
			schema = await GetSchemaAsync(name.AreaName, name.ObjectName).ConfigureAwait(false);
			lock (objLock)
			{
		...
		private async Task<string> GetDefaultDbSchemaAsync()
		{
			if (_defaultDbSchema.IsEmpty())
				_defaultDbSchema = await this.MdbContext.ExecuteAsync<string>(SqlServer.GetDefaultSchema);
		...
		private async Task<JMXSchema> GetSchemaAsync(string areaName, string objectName)
		{
		...
		async Task<JMXSchema> GetSchemaInternalAsync(string areaName, string objectName, int syncState)
		{
			MdbContext mdb = this.MdbContext;
			string sql = $"select Top 1 ID, ObjectSchema, SyncState from SysCat.V_SysSchemas where SchemaName = '{areaName}' and " +
		...
		private async Task DropDbSchemaAsync()
		{
			MdbContext mdb = this.MdbContext;
			var log = this.Logger;
