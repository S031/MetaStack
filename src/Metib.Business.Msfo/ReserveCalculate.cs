...
{
	public class ReserveCalculate : IAppEvaluator
	{
				...
				foreach (var dtype in dealTypes)
				{
					using (MdbContext mdb = new MdbContext(cs))
					{
						try
						{
							...
							exec workdb..mib_msfo_reserve_calc 
								@Date = '{date.ToString("yyyyMMdd")}'
								,@BranchID = {filial}
								,@DealType = '{dtype}'");
							string result = $"Success mib_msfo_reserve_calc {date} filial={branchID} deal type={dealType}";
