		...
		private static bool CompareAttributes(SQLStatementWriter sb, JMXSchema schema, JMXSchema fromDbSchema)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (found)
				{
					...
					{
						MdbTypeInfo ti = SQLiteHelper.TypeInfo[att.ServerDataType];
						if (!ti.FixedSize)
						{
							if (SQLiteHelper.TypeMapping.GetVariableLenghtDataTypes().Contains(att.ServerDataType) && att.DataSize.Size != att2.DataSize.Size)
