	...
	public class MapTableTest
	{
		...
		private void MapTableTests()
		{
			using (FileLog l = new FileLog("MapTableTest", new FileLogSettings() { DateFolderMask = "yyyy-MM-dd" }))
			{
				...
				{
					k = d.IndexOf(fixed_key_part + "500011");
				}
				stop = DateTime.Now;
				...
				l.Debug($"MapTable TryGetValue with {loop_count} runs index of = {k} whith fragmentation = {fragmentation} total time = {(stop - start).TotalMilliseconds} ms");
			}
		}
	}
}
